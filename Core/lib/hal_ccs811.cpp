...
CCS811Core::CCS811_Status_e CCS811Core::beginCore(I2C_HandleTypeDef *hi2c) {
    ...
    _hi2c = hi2c;    // Pull in user's choice of I2C hardware

    // Check the ID register to determine if the operation was a success.
    uint8_t readCheck;
    returnError = readRegister(CSS811_HW_ID, &readCheck);

    if(returnError != CCS811_Stat_SUCCESS) return returnError;

    if(readCheck != 0x81) { returnError = CCS811_Stat_ID_ERROR; }
...
CCS811Core::CCS811_Status_e CCS811Core::readRegister(uint8_t  offset,
    ...
    HAL_StatusTypeDef status = HAL_I2C_Mem_Read(
        _hi2c, _i2cAddr, offset, I2C_MEMADD_SIZE_8BIT, outputPointer, 1, 10);
    if(status != HAL_OK) { returnError = CCS811_Stat_I2C_ERROR; }
...
CCS811Core::CCS811_Status_e CCS811Core::writeRegister(uint8_t offset,
    ...
    HAL_StatusTypeDef status = HAL_I2C_Mem_Write(
        _hi2c, _i2cAddr, offset, I2C_MEMADD_SIZE_8BIT, &dataToWrite, 1, 10);
    if(status != HAL_OK) { returnError = CCS811_Stat_I2C_ERROR; }
...
CCS811Core::CCS811_Status_e CCS811::beginWithStatus(I2C_HandleTypeDef *hi2c,
    ...
    if(checkForStatusError() == true) return CCS811_Stat_INTERNAL_ERROR;

    if(appValid() == false) return CCS811_Stat_INTERNAL_ERROR;

    // Write 0 bytes to this register to start app
    uint8_t           txBuff[1] = {CSS811_APP_START};
    HAL_StatusTypeDef status
        = HAL_I2C_Master_Transmit(_hi2c, _i2cAddr, txBuff, 1, 10);
    if(status != HAL_OK) { return CCS811_Stat_I2C_ERROR; }
...
//
//****************************************************************************//
// Updates the total voltatile organic compounds (TVOC) in parts per billion
// (PPB) and the CO2 value Returns nothing
...
CCS811Core::CCS811_Status_e CCS811::enableInterrupts(void) {
    uint8_t                     value;
    CCS811Core::CCS811_Status_e returnError = readRegister(
        CSS811_MEAS_MODE, &value);    // Read what's currently there
    if(returnError != CCS811_Stat_SUCCESS) return returnError;
...
// Given a temp and humidity, write this data to the CSS811 for better
// compensation This function expects the humidity and temp to come in as floats
CCS811Core::CCS811_Status_e CCS811::setEnvironmentalData(float relativeHumidity,
    ...
    // envData[0] = ((rH % 1000) / 100) > 7 ? (rH / 1000 + 1) << 1 : (rH / 1000)
    // << 1; envData[1] = 0; //CCS811 only supports increments of 0.5 so bits
    // 7-0 will always be zero if (((rH % 1000) / 100) > 2 && (((rH % 1000) /
    // 100) < 8))
    //{
    ...
    envData[1] = 0;    // CCS811 only supports increments of 0.5 so bits 7-0
                       // will always be zero

    temp += 25000;    // Add the 25C offset
    // Split value into 7-bit integer and 9-bit fractional
    // envData[2] = ((temp % 1000) / 100) > 7 ? (temp / 1000 + 1) << 1 : (temp /
    // 1000) << 1; envData[3] = 0; if (((temp % 1000) / 100) > 2 && (((temp %
    // 1000) / 100) < 8))
    //{
...
//****************************************************************************//
//
//	The CCS811 no longer supports temperature compensation from an NTC
// thermistor. 	NTC thermistor compensation will only work on boards purchased
// in 2017. 	List of unsupported functions: 		setRefResistance();
// readNTC(); 		getResistance(); 		getTemperature();
//
//****************************************************************************//
...
CCS811Core::CCS811_Status_e CCS811::readNTC(void) {
    ...
    temperature
        = 1
          / (0.001129148 + (0.000234125 * temperature)
...
const char *CCS811::statusString(CCS811_Status_e stat) {
    ...
    switch(val) {
    case CCS811_Stat_SUCCESS: return "All is well."; break;
    case CCS811_Stat_ID_ERROR: return "ID Error"; break;
    case CCS811_Stat_I2C_ERROR: return "I2C Error"; break;
    case CCS811_Stat_INTERNAL_ERROR: return "Internal Error"; break;
    case CCS811_Stat_GENERIC_ERROR: return "Generic Error"; break;
