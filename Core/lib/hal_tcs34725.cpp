...
void TCS34725::write8(uint8_t reg, uint32_t value) {
    uint8_t tx_buff[2] = {static_cast<uint8_t>(TCS34725_COMMAND_BIT | reg),
...
void TCS34725::enable() {
    ...
    /* Set a HAL_Delay for the integration time.
    ...
    case TCS34725_INTEGRATIONTIME_2_4MS: HAL_Delay(3); break;
    case TCS34725_INTEGRATIONTIME_24MS: HAL_Delay(24); break;
    case TCS34725_INTEGRATIONTIME_50MS: HAL_Delay(50); break;
    case TCS34725_INTEGRATIONTIME_101MS: HAL_Delay(101); break;
    case TCS34725_INTEGRATIONTIME_154MS: HAL_Delay(154); break;
    case TCS34725_INTEGRATIONTIME_700MS: HAL_Delay(700); break;
    }
}
 ...
 */
bool TCS34725::begin(I2C_HandleTypeDef *hi2c, uint8_t addr) {
    _i2caddr = addr << 1;
    _hi2c    = hi2c;
    return init();
}
 ...
 *          Clear channel value
 */
void TCS34725::getRawDataOneShot(uint16_t *r,
                                 uint16_t *g,
                                 uint16_t *b,
                                 uint16_t *c) {
    //  if (!_tcs34725Initialised)
 ...
 */
uint16_t
    TCS34725::calculateColorTemperature(uint16_t r, uint16_t g, uint16_t b) {
...
void TCS34725::clearInterrupt() {
    uint8_t tx_buff[1] = {TCS34725_COMMAND_BIT | 0x66};
    HAL_I2C_Master_Transmit(_hi2c, _i2caddr, tx_buff, 1, 10);
}
